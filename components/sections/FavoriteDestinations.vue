...
<template>
  <div class="bg-tertiary/10 bg-opacity-30">
    <div class="container py-32 text-center">
      <div class="text-center mb-8 justify-center">
        <!-- <div class="border mb-2 "></div> -->
        <h1 v-if="route.params.member"
          class="font-semibold text-4xl md:text-7xl mb-6 title font-playfair-display tracking-wide">
          Favorites destinations
        </h1>
        <h1 v-else class="font-semibold text-4xl md:text-7xl mb-6 title font-playfair-display tracking-wide">
      ...
      <div class="w-3/4 mx-auto" v-if="items.length">
        <NuxtLink v-for="(item, index) in items" :key="index"
          ...
          :to="'/destinations/peru/' + item.url" @mouseenter="selectedItem = index; animateImage(index);"
